...
function splitIntoStanzas(lyrics: string[]) {
...
export function SongDetail({ song, category }: SongDetailProps) {
  ...
  return (
    <article className="mx-auto max-w-4xl px-5 pb-16 sm:px-8">
      <Link href="/" className="inline-flex items-center gap-2 pt-6 text-sm font-bold text-[var(--accent)] hover:text-[var(--ink)] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--accent)]">
      ...
      <header className="pt-12 sm:pt-16">
        ...
        <h1 className="mt-5 max-w-3xl font-display text-5xl font-semibold leading-[0.95] tracking-tight text-[var(--ink)] sm:text-7xl">{song.title}</h1>
        {song.melody && (
          <p className="mt-6 border-l-2 border-[var(--accent)] pl-4 text-lg font-semibold text-[var(--accent)] sm:text-xl">Melodi: {song.melody}</p>
        )}
      </header>
      ...
      <div className="mt-12 max-w-2xl border-t border-[var(--line)] pt-10 sm:mt-16 sm:pt-14">
        ...
        {song.notes && (
          <aside className="mt-12 rounded-2xl border border-[var(--accent)]/35 bg-[var(--accent-soft)] p-5 sm:p-6">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--accent)]">Toastmästarens notis</p>
            <p className="mt-2 text-lg font-semibold leading-7 text-[var(--ink)]">{song.notes}</p>
          </aside>
        )}
      </div>
    </article>
